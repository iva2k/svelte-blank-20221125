...
export function _getSession(user: DecodedIdToken | null): Session {
  if (user) {
    const { name, email, email_verified, uid, roles } = {
      name: '',
      roles: [],
      ...user
    };
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    return { user: { name, email: email!, email_verified: email_verified!, uid, roles } };
  }
  return { user };
}
